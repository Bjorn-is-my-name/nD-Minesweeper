...
#define RAYGUI_TABBAR_ITEM_WIDTH 160
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        if (GuiButton(closeButtonRec, "x"))
            closing = i;
#else
        if (GuiButton(Rectangle{tabBounds.x + tabBounds.width - 14 - 5, tabBounds.y + 5, 14, 14}, GuiIconText(ICON_CROSS_SMALL, NULL)))
            closing = i;
#endif
        GuiSetStyle(BUTTON, BORDER_WIDTH, tempBorderWidth);
        GuiSetStyle(BUTTON, TEXT_ALIGNMENT, tempTextAlignment);
        ...
        if ((guiFont.texture.id > 0) && (text != NULL))
        {
            ...
            for (int i = 0, codepointSize = 0; i < size; i += codepointSize)
            {
                ...
                if (guiFont.glyphs[codepointIndex].advanceX == 0)
                    ...
                    glyphWidth = ((float)guiFont.glyphs[codepointIndex].advanceX * scaleFactor + GuiGetStyle(DEFAULT, TEXT_SPACING));
